...
Illustrative, not exhaustive — the underlying state vocabulary is settled (`ADR-007`, 2026-07-25; see Shared Foundation), but the narrative copy shown here for each moment is not:
...
**Open:** the exact narrative copy for each state (the vocabulary itself is settled — see above); the mechanism for a customer-initiated request, remote from a walk-in visit.
