...
**Open:** the full revised status vocabulary; the exact shape of the estimate/approval record sequence; the recoverable-deletion mechanism itself — how long a hidden record stays recoverable and how it's restored, and whether/how a retention limit ever applies to sensitive unlock data specifically; product representation's shape within the timeline; Favorites' fate; inventory ownership (a complete external accounting and inventory system already exists, with no integration path today).
