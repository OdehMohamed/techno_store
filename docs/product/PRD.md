...
**Open:** the full revised status vocabulary; the exact shape of the estimate/approval record sequence; whether/how a retention limit ever applies to sensitive unlock data specifically; whether the recoverable-deletion/permanent-deletion mechanism settled for maintenance devices (`ADR-005`) generalizes to other entities on this timeline (repair episodes, purchases) or needs its own settling when a deletion need for those actually arises; product representation's shape within the timeline; Favorites' fate; inventory ownership (a complete external accounting and inventory system already exists, with no integration path today).
