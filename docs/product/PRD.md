...
- **Device.** A first-class entity, distinct from any single repair visit attached to it. Always anchored on a system-generated internal identifier; IMEI or serial number is used only as an optional strong-match hint when available, never as a required anchor. Devices are never silently merged on brand or model alone — any linking of two records to the same physical device requires human confirmation.
  The staff-facing intake workflow's shape and the matching/deduplication policy are both settled (`ADR-007`, 2026-07-25, including its Device Matching / Deduplication Policy addendum): one customer-first flow; likely-Device suggestions shown once customer/context is established, produced only by an exact IMEI/serial match or the customer's own previously-known Devices (brand/model/color never independently surfaces a candidate); an explicit staff choice between an existing Device and a new one; a cross-customer IMEI/device search reachable from inside that same flow, not a separate entry point. Matching evidence never blocks intake, and duplicate Devices from an unconfident non-match are an accepted cost — never a silent merge. No product-level decision remains open here — what's left (IMEI/serial normalization rules, query/index design) is implementation execution, tracked in `ADR-007`, not a product truth this document registers.
- **Visit.** One bounded repair episode, from intake through a natural conclusion — through delivery (`ADR-007`, 2026-07-25). What that conclusion actually was is its own settled fact, independent of both the Visit's operational status and any Estimate's outcome: a repair completed, no fault found, or a diagnosed fault the customer chose not to proceed with — kept deliberately separate so a completed repair and a separately-declined revision on the same Visit never get collapsed into one misleading answer. A Visit references the persistent Device it concerns and belongs to the Customer relationship the episode occurred under; Device itself carries no ownership or current-association concept, and a customer's authorization to see a Visit follows that Customer relationship, never a Device shared with someone else's Visit. Once Delivered, a Visit is closed and truthful — a returning device is always a new Visit, never a reopening of the old one.
...
Staff accounts also carry their own lifecycle status — active or inactive — entirely separate from role. This lets Admin suspend and later restore an employee's access without touching who they are or anything they've done: another instance of *Identity Persists, Attributes Change* (`METHODOLOGY.md`), alongside role itself. A staff session isn't restricted to a single device — there's no concrete business or security reason today that justifies the session-tracking infrastructure a single-device restriction would require — and there is no default automatic sign-out timeout for staff sessions, since shop devices commonly stay in active operational use throughout the day. Deactivation and a role change both take effect immediately, everywhere the change is observed, not on the account's next sign-in, consistent with treating staff-management as a real business-authority action.
...
**Open:** the phone-number-change migration and verification mechanism; shared-device staff identity switching or locking — some shop devices are shared between employees rather than belonging to one person, and a faster, safer way to switch or lock staff identity than re-entering full credentials each time is a confirmed need, with its secure mechanism (which may or may not involve inactivity-based locking while preserving the underlying session, distinct from a full sign-out) not yet designed.
...
The concrete shape of both is now settled (`ADR-007`, 2026-07-25): status is `In Progress → [Awaiting Approval] → Ready for Handback → Delivered`, with Delivered permanently terminal and Awaiting Approval an optional branch skipped when no customer decision is at stake. An Estimate is a whole-visit proposal (not per-problem/per-line-item), immutable once created, carrying at most one immutable decision; later revisions never overwrite earlier ones — the current Estimate is simply the latest in an ordered, server-timestamped sequence, never a separately tracked flag. A Visit's Final Amount Charged is a distinct fact from any Estimate it was approved against, answering what actually happened rather than what was authorized. The narrative and notification model built on top of this is now settled too (2026-07-26): six historical moment types tell a Visit's story, plus one transient, non-notifying current-condition presentation for an unresolved declined revision — see Relationship Lens for the full shape.

Some devices require a PIN or pattern lock to be captured for the repair to happen at all. This is sensitive data — captured only when genuinely needed, and protected by restricted access and isolated storage rather than general timeline visibility. Delivery and destruction are deliberately two different concepts: the Delivered event does not trigger any automatic purge of this data. The customer provided it voluntarily for the repair to happen, and it may still hold value for the device's future maintenance history — no automatic-destruction-at-delivery behavior was found valuable enough to justify losing that. Any future retention limit or permanent-deletion policy for this data is folded into the general business-authority question of permanent deletion (see Roles as Expertise, and the paragraph below), not treated as an automatic consequence of delivery.

Removing a record from normal use is recoverable by default — hidden, not destroyed — protecting the durable history this timeline exists to preserve. Permanent, irreversible deletion is a distinct, exceptional action, separate from ordinary removal. For maintenance devices specifically, this is now fully settled (`ADR-005`, 2026-07-23): ordinary removal (Archive) is available to any staff role, preserves the record and everything attached to it untouched, and is reversible indefinitely by Admin; permanent deletion requires the record to already be archived, is Admin-only, and is deliberately harder to reach than either of those, both technically (server-side enforcement) and in the UI. The same authority boundary — Admin-only for the permanent, irreversible action — is expected to generalize to other entities and to future business-authority actions like refunds, but each earns its own settling rather than being assumed from this one (see Roles as Expertise).
...
**Open:** whether/how a retention limit ever applies to sensitive unlock data specifically; whether the recoverable-deletion/permanent-deletion mechanism settled for maintenance devices (`ADR-005`) generalizes to other entities on this timeline (Visits, purchases) or needs its own settling when a deletion need for those actually arises; product representation's shape within the timeline; Favorites' fate; inventory ownership (a complete external accounting and inventory system already exists, with no integration path today).
...
The narrative and notification model is settled (2026-07-26, following `ADR-007`): **six historical moment types** tell the story of a Visit — the device being received; an Estimate awaiting the customer's decision (the first proposal, or a later revision — the same moment type either time); the customer's approval; the customer's decline; the device being ready for handback (worded differently depending on whether the technical work concluded as repaired, no fault found, or a diagnosed fault left unrepaired — never one generic message covering all three); and delivery. Content drawn from an Estimate (its proposed scope) is required to already be customer-safe language at the point it's authored, not translated or reinterpreted at display time — the exact meaning a customer approved has to stay historically stable. Only the Estimate-awaiting-decision and Ready-for-Handback moments trigger an outbound notification; the rest — including Delivered — are Relationship Timeline entries a customer sees if they check, not interruptions.

**A seventh condition is deliberately not a historical moment.** While a declined Estimate revision awaits a staff decision about previously-approved work (`ADR-007`'s Pending Revision Resolution), the Visit's current view may truthfully reflect that — calm, non-actionable, present only while the condition holds, gone once resolved, never an outbound notification. This is a live read of current condition layered over the historical sequence, not a new kind of Timeline entry — the same pattern already at work just below for *"Nothing in progress right now."* No elapsed-time threshold or business-hours concept gates it; the story only ever shows what's currently true.

Illustrative, not exhaustive:

- *"We've received your device. We'll be in touch once there's an update."* Nothing is expected of the customer yet, and nothing is claimed about what stage the work is actually at.
- *"Here's what we found: [scope]. Estimated cost: [amount]. Please let us know if you'd like us to go ahead."* This is the one kind of moment that deliberately breaks the calm default — a real decision the customer has to make, visually distinguished from everything around it (Design Principle 5, *Default Modes Break at Genuine Exceptions*).
- *"Your device is ready for pickup. Total: [amount]."* — worded differently depending on the technical outcome; never a claim about pickup availability the business hasn't settled.
...
**Open:** the mechanism for a customer-initiated request, remote from a walk-in visit.
