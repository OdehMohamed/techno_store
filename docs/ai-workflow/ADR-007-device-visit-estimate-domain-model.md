...
4. **Capability client.** New required Device selection at intake, Estimate creation, Awaiting Approval — all live, including on legacy Visits still open (§6). Rules remain unchanged from Phase 1–3. **Also requires:** Phase 2's compatibility-tab action set (Edit/Fixed/Archive for the In Maintenance group, Edit/Deliver/Archive for the Fixed group) groups literals by display compatibility only, not by workflow validity — a Visit sitting at `Awaiting Approval` can currently reach the legacy "Fixed" action from Phase 2's UI, bypassing the Estimate decision entirely. Harmless in Phase 2 (no real `Awaiting Approval` data exists in production until this phase ships), but Phase 4 must gate those legacy actions by actual status semantics, or replace them outright — not carry Phase 2's compatibility grouping forward as if it were a workflow rule.
