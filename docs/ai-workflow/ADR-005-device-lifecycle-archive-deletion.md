...
A one-time script (`scripts/migration/`, alongside the existing Phase 1C sensitive-data migration scripts, sharing `lib/admin.js`) backfills `recordState: 'active'` onto every `maintenanceDevices` document currently missing it. Dry-run by default, `--execute` to write, idempotent (already-migrated documents are skipped), paired with a read-only verify script — matching the existing `migrate-pass-a.js`/`verify-pass-a.js` pattern exactly. **Must be run against production, with a fresh backup taken first and coverage verified, after PR 1 merges and before PR 2 (the client cutover) ships** — the client's new queries would otherwise silently drop every pre-migration device from every tab.
...
**Between the two PRs:** production backup, run the migration with `--execute`, verify full coverage. Explicit product-owner approval gates this step — it's a live write across every real customer device record.

**PR 2 — client cutover and permission tightening, in the same change:** model + query updates (now safe, since every document has `recordState`), new composite indexes deployed, "Delete" swipe action becomes "Archive," new Admin-only `ArchivedDevicesPage` (Restore, Permanent Delete with typed confirmation), and — only now, in the same PR where the client stops relying on them — the parent/sensitive-subdocument/Storage client-delete permissions actually get removed.
