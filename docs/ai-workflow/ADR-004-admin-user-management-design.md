...
**Related:** `ADR-002-role-management.md` (Phase 2, Custom Claims), `docs/ai-workflow/archive/phase1-execution/PHASE1_IMPLEMENTATION_PLAN.md` §2, `docs/product/PRD.md` (Auth & Account Lifecycle), the 2026-07-25 Admin-area code review (findings B1/D1/D2/D3 — `NewUserAdminSide` non-functional, `create_user_account` orphaned, no account-creation mechanism exists anywhere).
...
**`NewUserAdminSide`'s disposition:** rebuilt as the create-account view inside the new staff-management surface (reusing its existing name/email/password/role fields, minus the photo picker), not left standing as a separate drawer-level screen, and not deleted outright without replacement. `lib/features/create_user_account/` (`CreateUserAccount`) is unrelated dead code — confirmed zero call sites anywhere in `lib/` — and is a plain removal, not something this feature repurposes.
