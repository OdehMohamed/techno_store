...
### 2026-07-24 — ADR-005 confirmed complete; Reception & Maintenance cleanup pass begins (dead code removed, PR #21)

**Decision:** With the production cutover verified end-to-end, the product owner confirmed ADR-005 (device lifecycle) fully complete and asked to explicitly carry forward the process that closed it — pressure-tested decisions, ADRs before code, executable verification at every layer, migration verification, and production smoke testing — as the standing rigor level for future consequential/risky work, not a one-off for this feature. `ADR-005-device-lifecycle-archive-deletion.md`'s status updated to Implemented.

Rather than assume the next area from the original phase sequencing (Reception & Maintenance → Admin → Customer → Retail), the product owner was asked to choose deliberately. **Decision: finish Reception & Maintenance's remaining open findings before moving to a new area.** Of the three remaining findings (dead code, employee attribution, intake form shape), the product owner agreed with the recommended order — dead code first (zero product decisions, purely mechanical), then employee attribution (a real decision conversation), then intake form shape (lower priority, not urgent) — matching the same "implementation-ready vs. needs-a-decision-conversation-first" lens used earlier for the route-guard/device-deletion split. Explicit instruction for the cleanup pass: remove dead code aggressively, including UI entry points that currently do nothing — "we'll almost certainly redesign them deliberately when their turn comes... I'd rather keep the codebase clean and carry forward only what is genuinely alive and intentionally part of the current product."

**Outcome (PR #21):**
- Removed `lib/features/manage_category/` entirely (six files): `ManageCategoriesPage` was a fully non-functional shell — its six grid tiles only toggled visual selection, no form ever rendered for any of them, and its `ManageCategoriesCubit`/`ManageCategoriesServices` were never wired up. Confirmed no other consumers via `grep` before deletion.
- Removed the unreferenced, fully commented-out `maintenance_list/view_model/maintenance_list_state.dart` (a pre-Cubit `ChangeNotifier`-era leftover, superseded by and confusingly co-named with the real Cubit-paired state file in `cubit/`).
- Removed the dead "Invoice" swipe action on the Delivered tab (empty `onPressed`, a TODO stub that silently did nothing on tap) and the already fully commented-out "Reopen" action.
- Removed five empty `onTap: () {}` drawer stubs — Favorite, Settings, Add new Product, the customer's "Maintenance (My Devices)" — plus the Manage Categories entry pointing at the now-removed page, from `MainDrawer2`.
- Removed the now-unused translation keys these removals left behind, in both `en.json` and `ar.json` (verified zero remaining code references before removing each key).

**Testing:** `flutter analyze` — 0 errors, unchanged pre-existing info/warning baseline. `grep` confirmed zero stray references to any removed file or symbol across `lib/`. Both translation JSON files validated as well-formed. No live/emulator testing — matches the lower rigor already applied to the similarly zero-decision `newDeviceMaintenance` route-guard fix (PR #17), per the risk-matched-rigor principle the product owner endorsed alongside the executable-testing-rigor confirmation above.

**Merged:** PR #21, squash-merged as `3c5fba8`. Feature branch `chore/reception-maintenance-dead-code-cleanup` deleted locally and remotely per `CONTRIBUTING.md` §9/§10.
...
### 2026-07-25 — Admin area reviewed; Staff Account Management chosen as first Admin decision thread; ADR-004 extended and PR 1 (backend) shipped

**Decision:** With Reception & Maintenance closed, product owner chose Admin as the next area — pressure-tested against alternatives (the deferred intake-form-shape question, the accepted bypass-the-UI rules-testing risk) rather than assumed from the original phase sequencing — on the strength of a real, live gap: no trusted mechanism exists anywhere to create a staff account, despite `PRD.md` settling "accounts are created directly by the Admin" as a real product decision.

**Outcome — Admin code-level review (no code changed):** routes, drawer, `new_user_admin_side`/`create_user_account`, and the shipped Staff Auth/`setStaffStatus` infrastructure inspected end-to-end against `PRD.md`, `ADR-004`, `OPEN_DECISIONS.md`, `BACKLOG.md`, and the deployed rules. Findings grouped by severity: (B) `NewUserAdminSide`'s submit path is fully non-functional (`AuthCubit.signUp`/`AuthServices.signUpWithEmailAndPassword` both commented out) and no Cloud Function ever existed to create staff accounts — ADR-004's original proposal never actually designed account creation, only activation and role-change; (D) `lib/features/create_user_account/` confirmed orphaned, zero call sites. One specific claim from an earlier session's orientation was explicitly re-verified and corrected rather than carried forward: `AuthCubit._listenToActivation`/`AuthNeedActivation` no longer exist in the codebase at all (confirmed via full-codebase grep) and `isActivated` is explicitly retired per `PRD.md` for customer accounts — there is no live authorization gap here, only stale documentation (`PERMISSIONS_MATRIX.md`'s `isActivated` section, `BACKLOG.md` item 10) describing code that predates the shipped `staffStatus` mechanism.

**Decision — design pass, before any code:** product owner asked for the creation mechanism, minimum data, atomicity/rollback behavior, trust boundary, initial `staffStatus`, and the surrounding management surface (list/filter/activate/deactivate) designed together, explicitly to avoid building the creation screen in isolation and redesigning it again immediately. Design proposed and pressure-tested across two rounds:
- **Round 1:** Cloud Function (`createStaffAccount`, Admin SDK) as the only viable mechanism — not just because the deployed `users/{uid}` `create` rule blocks it, but because client-side Auth user creation would sign the calling Admin's own session out and into the new account, independent of rules entirely. Product-owner decisions: Admin sets the initial password directly (v1, not assumed to be the ideal long-term lifecycle); new accounts start `active` by default (creation is itself the grant; `setStaffStatus` already covers "prepare now, deactivate immediately" for free); profile photo dropped from creation entirely; Admin may create another Admin through the same function, no separate mechanism, with a UI requirement that the Admin role be visually unmistakable; staff-list pagination deferred as premature infrastructure.
- **Round 2 (tightening, before implementation):** product owner identified two real gaps in the round-1 design before any code was written. First, the proposed compensating-delete test relied on temporary deny-write Firestore rules — invalid here, since Admin SDK writes bypass Security Rules entirely. Second, and more substantively: matching an idempotent-retry success on email+role content alone could silently resolve an unrelated pre-existing account as a false "success" if a coincidentally-matching new attempt hit an existing account. Both closed before implementation: a deterministic, doubly-gated test-only failure injection (`process.env.FUNCTIONS_EMULATOR === "true"` AND an explicit payload field — structurally unreachable in production, not just unreachable by convention) replaces the rules-based technique; a client-supplied `requestId`, anchored via a new `staffAccountCreationRequests/{requestId}` tracking doc claimed atomically before Auth creation, replaces content-matching as the identity model — role plays no part in the accept/reject decision at all under the corrected design.

`ADR-004` updated in place (not spun into a new ADR) with this full design: identity model, five-case resolution table, data-write ordering and atomicity boundary, compensating-delete-with-resume-fallback contract, and the four settled product decisions above — consistent with the document's own precedent of accreting dated, settled sections onto one coherent "Admin user management" record.

**Outcome (PR #24, backend only):** `createStaffAccount` added to `functions/index.js`, mirroring `setStaffStatus`'s two-part Admin+own-active-staffStatus authorization check. Executable verification against a local Firestore + Auth + Functions emulator (Java 21 via Homebrew, pointed at via `JAVA_HOME` for the session only — the environment's default JDK and global `firebase` CLI are both broken, consistent with the SSH issue noted at PR #21): 30/30 checks across normal creation (including Admin-creating-Admin), both caller-rejection paths, invalid-`type` rejection, duplicate-email rejection with both a matching and a differing role (proving role plays no part in the identity decision), idempotent same-`requestId` retry, `requestId`-reuse-with-different-content rejection, orphaned-Auth-user resume, and the injected-failure compensating-delete path. The first run of that last case caught a genuine bug, not just confirmed a design: the injected throw was originally placed outside the `try` block wrapping the Firestore batch, bypassing the compensating-delete `catch` entirely — the orphan survived and only the resume fallback masked it. Fixed (moved inside the `try`), rerun clean. `firebase.json` gained a minimal `emulators` block (none existed before) to run this verification at all; confirmed via direct inspection of `firebase-tools`' own source (`VALID_DEPLOY_TARGETS`, and the single place `FUNCTIONS_EMULATOR` is ever set, inside the emulator's own spawner) that this is inert for `deploy` and for the shipped app, per explicit product-owner request for that confirmation before merging.

**Testing:** `node --check functions/index.js` clean. Full 30-check executable emulator pass above — no automated test suite exists in `functions/` (matches this repo's established practice for Cloud Function verification, e.g. `setStaffStatus`/`permanentlyDeleteDevice`).

**Merged:** PR #24, squash-merged as `2f6ed4c`. Feature branch `feat/staff-account-creation-backend` deleted remotely (local branch cleanup was superseded by re-syncing `main` directly, after the same broken-SSH environment issue noted at PR #21 interrupted `gh`'s local post-merge housekeeping — the merge itself completed via GitHub's API, confirmed independently before any local git state was touched).

**Explicitly not decided in this session:** PR 2 (the Staff Management client vertical slice: list/filter screen, `createStaffAccount`/`setStaffStatus` wiring, `NewUserAdminSide`/`create_user_account` removal) — up next, per the same two-PR sequencing precedent as ADR-005. Role-change, audit-log in-app visibility, and the Admin dashboard/reporting area all remain deliberately out of scope, per `ADR-004`'s own "Explicitly not decided" section.

---

### 2026-07-25 — Staff Management client shipped (PR 2 of 2); Staff Account Management thread closed

**Decision:** Product owner reviewed PR 1's diff and verification results and gave the go-ahead to implement PR 2 without a further design review round, since the product decisions, ADR, backend contract, and failure-handling behavior were already settled and merged. Explicit scope: Admin-only staff list/filter, creation through `createStaffAccount`, activate/deactivate through the already-shipped `setStaffStatus` (reused, not modified), replace `NewUserAdminSide`, remove `new_user_admin_side` and `create_user_account` once superseded — with role-change, audit-log visibility, and the Admin dashboard explicitly out of scope. Product owner also required Android-emulator verification specifically (not Chrome/web), citing the standing precedent from Staff Auth/ADR-005/ADR-006, so a Staff-Management-specific bug wouldn't get confused with an unrelated web-platform issue.

**Outcome — a real correction found and fixed during implementation, not left for review:** while wiring up imports, re-grepping for the actual class name (rather than trusting the earlier review's grep for the filename) showed `lib/features/create_user_account/` (`CreateUserAccount`) is **not** dead code — it's the live screen `MainScreen` renders on `AuthNeedsProfileCompletion`, the customer phone-OTP profile-completion step, unrelated to staff management entirely. The original Admin-area review (same-day, earlier session) had searched for `CreateUserAccountView` (the filename) instead of the actual class. Caught before any deletion. `ADR-004` and `BACKLOG.md` corrected in place rather than silently fixed; `create_user_account` left untouched. `lib/core/widgets/main_drawer.dart` — a separately, correctly confirmed-dead file (`BACKLOG.md` item 7) — was removed instead, as a directly necessitated side effect of removing `NewUserAdminSide` (it held the only remaining reference to that class).

**Outcome (PR #25, client):** new `lib/features/staff_management/` (model/services/cubit/view — list/filter screen, creation bottom sheet with a visually-distinct Admin role option, activate/deactivate wired to `setStaffStatus` with a confirmation dialog on deactivate only). Routes/drawer updated (`AppRoutes.staffManagement` replaces `createAccountAdminSide`). Small additive change to the shared `EmptyStateWidget` (`maintenance_states.dart`): new optional `title`/`subtitle` overrides, existing callers' defaults unchanged.

**Executable verification, real Android emulator against local Firestore + Auth + Functions emulators** (not Chrome, per explicit product-owner instruction): sign-in as a seeded Admin, drawer shows "Staff Management" (Admin-only), list/filter loads real data, Admin role tile shows the designed amber visual distinction, created a new Reception account (`createStaffAccount` succeeded, list auto-refreshed with no manual reload), deactivated it (confirmation dialog fired with correct forced-signout copy, `setStaffStatus` succeeded, "Inactive" filter correctly isolated it), reactivated (no confirmation dialog, by design). One real issue the smoke test surfaced: the empty-state screen showed maintenance-devices copy ("No devices found") on the staff list — fixed via the `EmptyStateWidget` change above, re-verified.

**Environment note, not a product/app finding:** getting a clean Android run required real troubleshooting, fully isolated to test tooling. Root cause of repeated false failures: `pkill -f "flutter run"` never matched anything all session (`flutter_tools.snapshot run -d chrome`/`-d emulator-5554` doesn't contain the literal substring "flutter run"), so stale `flutter run`/Chrome processes from earlier attempts kept accumulating and were sometimes driven by mistake instead of the current, fixed build. Separately, an interim debug-harness fix (`clearPersistence()` + `terminate()` added to the temporary emulator-pointing code in `main.dart`) introduced a self-inflicted `[cloud_firestore/failed-precondition] "client already terminated"` bug, reverted in favor of the plain documented `useFirestoreEmulator`/`useAuthEmulator` pattern. Both fully isolated to non-shipped debug wiring (`main.dart`, a temporary Android `network_security_config.xml`) — confirmed via `git status` showing zero diff on either before this PR was opened.

**Testing:** `flutter analyze` — 44 issues, identical to the pre-existing baseline (verified via `git stash` A/B comparison) — zero new issues. Full Android executable smoke test above.
...
### 2026-07-25 — Device / Visit / Estimate domain model settled and ratified as ADR-007; documentation reconciled

**Decision:** The Foundational decision-work cluster opened in the prior entry (device-identity/matching/intake-lookup) expanded, by the product owner's own direction as each pass concluded, into four sequential product-discovery passes covering the full domain: Device identity and intake matching; Visit boundaries once Device is persistent; the status vocabulary; and the Estimate/Approval record shape — resolved together in one continuous conversation rather than the originally-anticipated separate threads, since each pass's settled decisions became direct context for the next. Once all four were product-settled, the product owner asked for the result turned into a concrete architecture/schema proposal, explicitly to be pressure-tested before ratification, not implemented on first draft.

**Outcome — product decisions settled (pure discovery, no schema at any point):**
- Device is a persistent, first-class entity anchored on a system-generated internal ID; IMEI/serial is matching evidence, never identity; matching asymmetry ratified as a standing rule (never silently merge; unmatched-new is always available; duplicate Devices from an unconfident non-match are an accepted cost).
- No formal Device-ownership/current-association model — the minimum relationship is Visit→Customer (the relationship a service episode occurred under) and Visit→Device (the physical device involved), one-directional, no Device→Customer pointer; customer-facing authorization follows the Visit/Customer relationship, never a Device shared across customers.
- One customer-first intake flow (not two entry points), with likely-Device suggestions after customer/context is established, an explicit staff choice between existing/new Device, and a cross-customer IMEI/device search as an in-flow escape hatch — matching never blocks intake.
- A Visit is one bounded intake-to-delivery episode; Delivered is permanently terminal; a returning device always creates a new Visit, never a reopen.
- Status vocabulary settled: `In Progress → [Awaiting Approval] → Ready for Handback → Delivered`, Awaiting Approval an optional branch, Ready for Handback redefined to mean pickup-readiness regardless of repair outcome (not merely "Fixed").
- Estimate is a whole-visit (not per-line-item) proposal, immutable once created, carrying at most one immutable staff-recorded decision; a Visit may have an ordered sequence of Estimates, with "current" derived positionally rather than a mutable flag; Final Amount Charged is a distinct fact from any Estimate.

**Outcome — architecture (`ADR-007`, three pressure-test rounds):** New `devices` collection (staff-only read, no customer access ever — the structural fix for cross-customer exposure through a shared Device); `maintenanceDevices` kept as the physical Firestore path with "Visit" as the domain concept, not renamed; `deviceId` permanently absent on pre-cutover Visits (rejecting both a false-distinctness one-Device-per-legacy-Visit backfill and a false-merge shared-placeholder-Device alternative), immutable and creation-only once the new-world invariant applies; legacy `price` left semantically unmigrated, with a new `finalAmountCharged` field carrying its own rules-enforced lifecycle (writable only into Ready for Handback, frozen at Delivered); Estimate `createdAt` enforced as `request.time` with `estimateId` as a deterministic tie-breaker, closing a client-clock ordering-integrity gap found during review; Estimate creation restricted to Maintenance+Admin (decision-recording staff-wide); a corrected 8-phase rollout (bridge client → confirmed adoption → capability client → confirmed adoption → rules tightening → data migration) fixing an app-distribution/rules-deployment race found in an earlier draft.

**Documentation reconciled (this entry's own scope):** `docs/ai-workflow/ADR-007-device-visit-estimate-domain-model.md` created, Status "Ratified. Not yet implemented." `PRD.md` (Core Entities gained a Visit entity bullet, Device's Open line narrowed to just the matching algorithm, two separate stale "status vocabulary is open" claims found and narrowed to narrative-copy-only), `OPEN_DECISIONS.md` (Device/intake entries merged and narrowed, status/estimate entries closed or narrowed, five new Revisitable Concepts entries added for what `ADR-007` considered and deliberately didn't build), and `ROADMAP.md` (Foundational group marked resolved, two remaining narrow threads moved to Self-Contained) all updated to match.

**Testing:** Documentation-only change; no schema, Firestore rules, migration execution, or client code touched; `flutter analyze` not affected.
...
### 2026-07-25 — Technical Finding / Pending Revision Resolution correction folded into ADR-007, surfaced by Status Narrative Copy discovery

**Decision:** Before writing customer-facing narrative copy, the product owner asked whether the underlying domain model could actually answer what a customer's Timeline needs to represent — deliberately pressure-testing facts, not starting from the existing schema. Two real gaps surfaced and were resolved across several rounds of pressure-testing (checking `MaintenanceListServices`/`MaintenanceDeviceModel`'s actual current behavior, not assuming), then corrected directly into `ADR-007` rather than deferred or worked around at the narrative layer.

**Outcome — gap 1, Technical Finding:** `status` and Estimate outcome cannot reliably answer what the technical work established. The break case: a Visit where an earlier Estimate was approved and completed, but a later, separately-proposed revision was declined — reading only the latest Estimate's outcome would misreport a successfully-repaired Visit as unrepaired. Checking the current code confirmed `fixedAt` is already set through one dedicated, explicit staff action (`updateDeviceAsFixed`), never inferred from status or Estimate history — a sound behavioral pattern worth keeping, but its name and binary "was it fixed" framing can't honestly cover a real third case discovery surfaced: diagnosis with no fault found, which is neither "repaired" nor "the customer declined anything." Settled: a new, independent Visit-level **Technical Finding** (*Repaired* / *No Fault Found* / *Fault Found, Not Repaired*), paired with a **technical-work-concluded-at** timestamp replacing `fixedAt`'s narrower assumption — both set via the same explicit-action pattern, storage field names deliberately left unsettled. No general "reason repair didn't happen" field was added — when the finding is *Fault Found, Not Repaired*, the reason is already answerable from the existing Estimate sequence (a declined Estimate → customer declined); the rarer "never formally offered because not viable" case is left as free text, not formalized, since it hasn't earned structure.

**Outcome — gap 2, Pending Revision Resolution:** pressure-testing delayed resolution, app restart, and staff handoff against a decline-with-prior-approval scenario confirmed the four ratified status values are genuinely incomplete, not just under-narrated — during a real delay between the decline and staff's resolution, none of the four (`Awaiting Approval`, `In Progress`, `Ready for Handback`, `Delivered`) is honestly true. This traced back to a gap in `ADR-007`'s own original §7 text, which silently assumed instantaneous resolution without ever stating the transition that gets a Visit there. Considered and rejected: a fifth linear status value (the condition has no meaning outside this one scenario, and would risk future status proliferation against the PRD's own "kept deliberately simple" discipline) and a fully generic "blocked pending staff decision" abstraction (premature — only one concrete instance exists so far). Settled: a narrowly-scoped **Pending Revision Resolution** fact, existing only when a later revision is declined after an earlier Approved Estimate, resolving to *continue* (→ In Progress) or *stop* (→ Ready for Handback) with who/when recorded, taking precedence over `status` while present rather than requiring `status` itself to be overridden or falsified. Explicitly confirmed independent of Technical Finding — "stop" doesn't imply nothing was repaired (the original scope may already have been completed before the revision was even proposed), and "continue" doesn't guarantee success either.

**Outcome — a precision fix, not a new finding:** `ADR-007` §5's Final Amount Charged ceiling already read "current Approved Estimate," which — checked directly against the walkthrough — was already correctly specific enough (the *governing* Approved Estimate, not merely the positionally-latest one). No behavioral correction was needed there, only clarified wording distinguishing "current" (positional) from "governing Approved" (outcome-specific), since a later declined revision can be positionally current while an earlier approval remains the real authorization boundary.

**Verification method:** an exhaustive end-to-end walkthrough of every legal Visit path (zero Estimates, one Estimate, revised Estimates, decline before any approval, decline after prior approval resolved both ways, no-fault-found, successful repair) confirmed all of "what's happening / what's it waiting on / technical finding / conclusion time / Estimate decisions / Ready-for-Handback outcome / amount due / delivered" are answerable directly in every path, with no fragile inference, before the correction was folded in.

**Documentation reconciled:** `ADR-007` corrected in place (§4: Technical Finding + technical-work-concluded-at, `fixedAt` removed from the "unchanged" field list; §5: ceiling-invariant wording precision; §7: Pending Revision Resolution, independence from Technical Finding, corrected transition text; Consequences: both deliberate non-generalizations plus the new not-yet-decided write-authority question added; Status line updated). `PRD.md`'s Visit entity bullet corrected — its prior "a completed repair, or a declined estimate" phrasing was itself an oversimplification the new finding disproved. `OPEN_DECISIONS.md`/`ROADMAP.md` checked and found not directly affected — neither referenced `fixedAt` or the old two-outcome assumption, so left untouched rather than edited for its own sake. `CURRENT_TASK.md` updated in both the Active task and "What is NOT yet decided" sections.

**New item surfaced, not resolved:** write authority (who may set Technical Finding, technical-work-concluded-at, and resolve a Pending Revision Resolution) was not part of what this pass was asked to settle — flagged in `ADR-007`'s Consequences rather than silently decided alongside everything else.

**Testing:** Documentation-only change; no schema, Firestore rules, migration, or client/UI code touched; `flutter analyze` not affected.
...
### 2026-07-26 — Pending Revision Resolution corrected to a per-Estimate embedded shape; three integrity refinements ratified

**Decision:** Continuing Status Narrative Copy discovery surfaced a real structural gap in the prior day's correction before candidate copy could be finalized: the ratified narrative causal test ("was the Fault Found, Not Repaired outcome actually attributable to a customer decline") depends on unambiguously associating a specific declined Estimate with its own resolution — and the model as written never actually established that a Visit-level "Pending Revision Resolution" fact preserves that association correctly across more than one occurrence.

**Outcome — the multiplicity gap confirmed real:** once multiple Estimate revisions are legitimately possible on one Visit, the qualifying condition (a later revision declined while an earlier Approved Estimate exists) can recur — e.g., `#1` approved → `#2` declined, resolved *continue* → `#3` (a further revision) declined, resolved *stop*. A single mutable Visit-level fact would overwrite `#2`'s own resolution the instant `#3`'s occurrence landed, permanently losing who decided what and when — a direct violation of the same Sequences-Carry-Meaning discipline this entire domain-model effort exists to enforce for `status` and `price`.

**Outcome — corrected shape:** resolution now lives directly on the qualifying Estimate itself — three new fields (`resolutionOutcome`, `resolvedByUid`, `resolvedAt`), present only on a Declined Estimate that qualifies, transitioning exactly once (`null → 'continue'/'stop'`) then frozen, mirroring `outcome`'s own one-time-transition shape as a second, independently-gated transition on the same document. The association is structural, not referential — there's nothing to point back to, since the fact lives on the record it concerns — which preserves every occurrence independently for free and avoids a second sequence whose only job would be pointing at the Estimate sequence that already exists.

**Outcome — three further integrity questions pressure-tested and ratified alongside the shape correction, before it was folded in:**
1. **No new Estimate may be created while a qualifying declined Estimate is unresolved** (app-layer invariant, v1). Pressure-tested against a real shop justification (staff wanting to propose a second newly-discovered issue before resolving the first) and found it doesn't survive — Estimates are already whole-visit, not per-line-item, so co-discovered issues should already be combined into one proposal; and resolving an existing decline is a cheap, customer-independent technical call, not a real bottleneck. Walking `#3` through Pending/Approved/Declined while `#2` sits unresolved showed genuine confusion in every case — two independently-unresolved conditions (a customer decision and a staff decision) with no single truthful answer to what the Visit's operative state is.
2. **`decidedAt` and `resolvedAt` are now server-authoritative** (`== request.time`), joining `createdAt`. Previously treated as passive provenance; now load-bearing, since the narrative model composes a decline and its resolution into one moment or inserts an interim reassurance based directly on the elapsed time between them — a client-supplied value could silently corrupt that behavior.
3. **Qualification is history-anchored, not live-state-anchored** — a Declined Estimate qualifies only if an Approved Estimate exists earlier in the immutable ordered sequence *as of that point*, never "does one exist whenever someone happens to check." Walked three ordering cases to confirm this gives a stable, correct answer regardless of when evaluated — and found it's mutually reinforcing with item 1: once new-Estimate-creation is blocked during an unresolved window, nothing relevant to the qualifying question can change during that window anyway, which is exactly why the history-anchored specification (not "live state that happens to currently agree") is the robust one to keep.

**A concern from the prior day's entry is resolved by construction, not merely deferred:** the transaction/consistency risk of a decline write leaving a Visit without its resulting resolution fact no longer exists as a separate question — resolution lives on the same document the decline write already touches, so there's no second fact a partial write could omit.

**Documentation reconciled:** `ADR-007` §7's correction rewritten in place (embedded shape, history-anchored qualification, no-new-Estimate invariant, "resolved by construction" note replacing the old implementation concern); §8 gained the three new fields plus paragraphs on server-authoritative `decidedAt`/`resolvedAt` and the second one-time-transition shape; Final authorization matrix's two Visit-level "Pending Revision Resolution" rows replaced with a `resolution recording` row on `.../estimates/{id}` and an amended `create` row reflecting the new precondition; Consequences updated to reflect the resolved-by-construction status. `CURRENT_TASK.md` updated in both the Active task and "What is NOT yet decided" sections.

**Testing:** Documentation-only change; no schema, Firestore rules, or client/UI code touched; `flutter analyze` not affected.

**Merged:** Committed directly to `main` (documentation only, no PR, matching prior `ADR-007` precedent).

**Explicitly not decided/not implemented in this session:** `ADR-007` implementation itself — still not started. Two implementation-adjacent items remain recorded, not designed: IMEI/serial normalization; the customer-known-Devices query/index design. The domain and workflow model is now final at the product-decision level — Status Narrative Copy discovery resumes next, re-evaluating the seven-moment model against the corrected per-Estimate resolution semantics.

---

### 2026-07-26 — Status Narrative Copy settled: six historical moments plus a transient live-state presentation, not seven persisted moments

**Decision:** Continuing narrative discovery against the corrected per-Estimate resolution model surfaced one last product question before ratification: what determines when the "declined revision awaiting staff resolution" condition becomes customer-visible. An initial elapsed-time threshold (resolved past the calendar day following the decline) was proposed, then rejected on the product owner's own pressure-test — the boundary was arbitrary relative to any real product fact, and produced materially different real waits (Monday 09:00 vs. Monday 23:59) for the same threshold crossing.

**Outcome — the conceptual question resolved first, not assumed:** whether the Relationship Timeline is strictly historical event log, or can also present current derived condition. Resolved using the PRD's own existing, already-settled precedent — *"When nothing is currently active, the experience says so plainly — 'Nothing in progress right now' — with history still quietly present beneath it"* — direct textual evidence the customer experience already layers live current-state reads on top of the historical sequence. This is not a new kind of thing being introduced.

**Outcome — Model B chosen over a delayed-moment design:** while a qualifying declined Estimate revision remains unresolved, the customer's view of the Visit may truthfully reflect that whenever viewed — calm, non-actionable, present only while true, gone once resolved, never an outbound notification. This eliminates the threshold entirely (there's no boundary to cross; the presentation is a live read, not a timer), naturally reproduces "compose when resolution is near-immediate" as a side effect of the customer being unlikely to check during a short window rather than as a rule that needs stating, and resolves a real specification gap the threshold-based design never actually closed (whether the interim moment, once shown, would persist permanently in history after resolution — Model B never has this problem, since it was never a historical entry). No new persisted fact required — the same query already established for the no-new-Estimate invariant and the governing-Approved-Estimate check.

**Outcome — terminology corrected:** what had been called a "seven-moment model" is precisely six historical narrative moment types (Received; Estimate awaiting decision, covering both first proposal and later revisions; Approved; Declined; Ready for Handback, branching on Technical Finding; Delivered) plus one conditional, non-historical live-state presentation — not a seventh moment. Ratified candidate copy for all six carries forward unchanged from the prior round's wording-precision pass (customer-safe, no unsupported claims about operational stage, technical rigor, or pickup availability).

**Outcome — the per-state notification documentation gap (open since the start of this discovery thread) is now closed:** Estimate-awaiting-decision and Ready for Handback trigger an outbound notification; Received, Approved, Declined, Delivered, and the live-state presentation never do — recorded in `PRD.md` directly rather than existing only in conversation history.

**Documentation reconciled:** `PRD.md` — Shared Foundation → The Relationship Timeline gained a settled-model summary sentence and lost its "narrative copy open" marker; Relationship Lens → The Relationship, As Experienced rewritten with the six-plus-one model, the per-state notification table, and corrected illustrative copy (replacing examples already known to overclaim — "in queue for diagnosis," "checked thoroughly," "come by whenever works for you" — with the wording ratified across the preceding rounds); both "Open" markers in this area removed. `OPEN_DECISIONS.md`'s "status vocabulary's narrative copy" entry removed entirely (fully earned). `ROADMAP.md`'s Self-Contained group no longer lists it, and the Foundational-group resolution note corrected to state both narrower threads it once pointed at are now fully settled, not merely relocated. `CURRENT_TASK.md` updated in both the Active task and "What is NOT yet decided" sections.

**Nothing new flagged during reconciliation** beyond the terminology correction itself — no inconsistency found between what was ratified across the narrative-copy rounds and what `ADR-007` independently states.

**Testing:** Documentation-only change; no schema, Firestore rules, or client/UI code touched; `flutter analyze` not affected.

**Merged:** Committed directly to `main` (documentation only, no PR, matching prior `ADR-007` precedent).

**Explicitly not decided/not implemented in this session:** implementation of either `ADR-007` or the narrative/notification model — neither has started. The entire Foundational decision-work thread opened at the start of this cluster (Device identity and matching, Visit boundaries, the status vocabulary, Estimate/Approval, and now the customer-facing narrative built on top of all of it) is product-settled end to end. Next step is the product owner's call on what comes next — implementation, or a different product area.

---

### 2026-07-26 — `ADR-007` Phase 1 implemented, reviewed, merged, and deployed to production (PR #26)

**Decision:** With the Foundational decision work fully product-settled, the product owner chose to move from design into implementation rather than open a new decision thread, specifically Phase 1 of `ADR-007`'s own 8-phase rollout — additive-only, unreachable by the shipped client, the same low-risk shape that made `ADR-004`/`ADR-005`'s first PRs safe.

**Outcome — implementation:** two new Firestore rule surfaces (`devices/{deviceId}`; `maintenanceDevices/{deviceId}/estimates/{estimateId}`, nested under the existing, untouched `maintenanceDevices` match block) plus a new `isMaintenanceOrAdmin()` helper. A rules-refinement pass before any code was written added canonical field-type validation, exact key-set bounding via `keys().hasOnly()`/`hasAll()`, provenance fields pinned to `request.auth.uid`, and `diff().affectedKeys().hasOnly(...)`-bounded one-time transitions rather than per-field equality checks alone. Zero Dart/`lib/` changes — confirmed via `git status` and an unchanged `flutter analyze` issue count.

**Outcome — permanent test infrastructure:** `test/firestore-rules/` established as reusable regression infrastructure (`@firebase/rules-unit-testing` + Node's built-in test runner, pinned lockfile), explicitly scoped as the harness Phase 6 and later rule changes extend rather than rebuild. Two real bugs found and fixed before this ever reached production: an unguarded `request.resource.data` field access inside the resolution-recording branch that threw when evaluated as an irrelevant sibling of an ordinary approval (fixed by extending this file's own pre-existing `recordState`-style `.get(field, default)` pattern); and a cross-file `clearFirestore()` race from Node's default concurrent test-file execution against one shared emulator project (fixed with `--test-concurrency=1`, documented so it isn't later removed for speed).

**Outcome — toolchain re-verified, not assumed:** the global `firebase` CLI remains broken (same template `ENOENT` as prior sessions); `firebase-tools@15.24.0` (pinned) now hard-requires Java 21+, stricter than earlier in this project's history — confirmed via an actual failed emulator start on the environment's default JDK 19, then a successful one after prepending Homebrew's `openjdk@21` to `PATH` (not just `JAVA_HOME`, which alone was insufficient).

**Outcome — independent security review before merge, per explicit request:** a full correctness/authorization review against a 10-point checklist (customer-access boundaries, actor/timestamp/provenance pinning, optional-field safety, `declineReason` integrity, cross-customer exposure, helper-scope, schema bounding, harness-masking risk, failure-case coverage, regression-baseline sufficiency) found zero Must Fix and zero blocking Should Fix. One claim — that the existing `maintenanceDevices` rules were untouched — was verified directly via a diff filtered to additions/deletions (100% additions, zero deletions) rather than asserted from memory, confirming the small 8-case regression subset was sufficient without rerunning the historical 74-case `BACKLOG` #0a matrix. Two Nice-to-Have items (query-shaped customer-read coverage; an end-to-end authorized lifecycle test proving the individually-tested transitions actually compose) were taken before merge as cheap confidence improvements, not vulnerability fixes — suite grew from 82 to 86, nothing new surfaced.

**Outcome — merge and production deployment:** `gh pr merge --squash --delete-branch` failed on its local SSH-dependent housekeeping step (the same known-broken-SSH pattern as every prior merge on this repo) but the merge itself succeeded via GitHub's API — verified via `gh pr view 26 --json state,mergedAt,mergeCommit` before proceeding, per this project's own established practice, rather than concluding failure from the error text. Local `main` resynced via HTTPS fetch + `git merge --ff-only` (clean fast-forward, `c00af0c..4f9e853`). `firestore.rules` deployed alone via `firebase deploy --only firestore:rules --project technostore-v2` from that clean, merged checkout — confirmed via deploy output ("rules file firestore.rules compiled successfully," "released rules... to cloud.firestore," "Deploy complete!"). No Functions, Storage rules, indexes, or client build touched, by explicit scope and by the `--only` flag.

**Testing:** `test/firestore-rules/` — 86/86 passing (final count, after the pre-merge review additions), against the local emulator, before deployment. No application/client testing needed — zero client code changed.

**Merged:** PR #26, squash-merged as `4f9e853bb2a7bd47908aaf94cdee779ab690a95f` to `main`. `feat/adr007-phase1-devices-estimates` deleted both locally and remotely (remote deletion confirmed via a 404 on `gh api repos/.../branches/...`).

**Deployed:** `firestore.rules` live in production (`technostore-v2`) as of this entry. The new `devices`/`estimates` surfaces remain unreachable by the currently-shipped client — no Dart code references either collection anywhere in `lib/`, so this deployment has no observable effect on the running app.

**Explicitly not decided/not implemented in this session:** `ADR-007` Phases 2–8 — not started. Two implementation-adjacent items remain recorded, not designed, in `ADR-007`: IMEI/serial normalization; the customer-known-Devices query/index design. Stopping here per explicit instruction, before beginning Phase 2 (the bridge client).

---

### 2026-07-26 — `ADR-007` Phase 2 (Bridge Client) implemented, reviewed, merged (PR #27)

**Decision:** Continuing `ADR-007`'s rollout, the product owner chose Phase 2 next — the Bridge Client — with one rollout clarification made explicit before planning began: staff will not receive Phase 2 as an independently deployed production update while later phases are built. They stay on the current production version until the whole ADR-007 body of work is ready for one coordinated transition. This meant the dual-vocabulary read/grouping design didn't need to be hardened against a prolonged mixed-version production window — it's a compatibility foundation for the phases ahead, not active protection for a rollout scenario that won't happen.

**Outcome — planning pass grounded in the actual client:** a full read-only inspection of `lib/features/maintenance_list/` and `new_device_maintenance.dart` mapped every status-coupled site — 4 exact-string `switch` statements (tab colors/icons/empty-state/slidable-actions), one Firestore query (`_deviceTabQuery`'s single-literal `isEqualTo`), two live write paths (`updateDeviceAsFixed`, `deliverDevice`, both legacy-only), one dead write path (`updateDeviceStatus`, zero UI callers, re-confirmed twice), and two UI-conditional literal comparisons in the edit screen (`fixedDetailsSection()`'s gate). Explicit pressure-test on visible wording concluded Phase 2 should be maximally invisible — same tab labels, colors, icons, dialogs throughout — since introducing new wording before Phase 4's real workflow exists would be pure risk for zero benefit.

**Outcome — implementation:** `lib/core/utils/device_status.dart` created as the single source of truth — three compatibility groups (`inMaintenanceGroup`, `fixedGroup`, `deliveredGroup`) mapping legacy and ADR-007 literals by operational meaning, plus a case-insensitive `isInGroup()` check. Every prior exact-string comparison across `inner_maintenance_list.dart` and `new_device_maintenance.dart` now routes through it. `_deviceTabQuery` moved from `isEqualTo` to `whereIn` on a `statusGroup: List<String>`, so one tab matches every literal in its group; `DeviceCard` now receives each device's own `status` instead of a blanket tab-level value, since a tab can now legitimately hold devices from either vocabulary. Zero write-path changes beyond routing the same legacy constants through the new helper — confirmed via full cubit→service→Firestore call-chain tracing, not just grep: the edit screen's save path preserves an existing device's status unchanged through a `merge: true` write, never introducing or resetting a literal.

**Outcome — Android emulator smoke test, not just static analysis:** local Firebase emulators seeded with a deliberate mix of legacy and ADR-007 literals, driven through a real staff sign-in on `emulator-5554`. Confirmed all three tabs group both vocabularies identically (same color/icon regardless of underlying literal), the edit screen's `fixedDetailsSection()` correctly appears for a `Ready for Handback` device, and — the core invariant — both live write paths, exercised starting from a *new-vocabulary* source status, still wrote only the legacy literal (`Ready for Handback` → Deliver → `status: "Delivered"`; `Awaiting Approval` → Fixed → `status: "Fixed"`), confirmed by reading the Firestore emulator directly rather than trusting the UI alone. Two unrelated pre-existing bugs surfaced and worked around without touching app code: a stale cached Auth session left over from an earlier unrelated smoke-test session, and `int.parse(colorHex, radix:16)` expecting hex without a `#` prefix (a seed-data mistake, not a code defect). Two temporary local-only patches (a `--dart-define`-gated emulator hookup in `main.dart`, a debug-only `usesCleartextTraffic` manifest flag) were needed to make the test possible at all and were fully reverted before commit — confirmed via `git status` showing only the intended diff.

**Outcome — independent compatibility-focused review before merge:** traced every claim through actual code rather than grep alone. Confirmed `DeviceStatus` is the sole comparison mechanism (the only literal-string hits remaining are `.tr()` display labels, not comparisons); confirmed the Awaiting-Approval-reaches-legacy-Fixed action path is real but causes no data corruption and is unreachable in Phase 2's actual production window; confirmed `whereIn` needs the identical composite index shape `isEqualTo` did, so this PR introduces no new index requirement in kind. Zero Must Fix findings. Two Should Fix / Nice to Have items taken before merge as cheap, directly-adjacent verification rather than deferred indefinitely:

1. **Deployed-index comparison.** `firebase firestore:indexes --project technostore-v2` diffed against the checked-in `firestore.indexes.json` surfaced real drift: 5 deployed indexes absent from the file (4 are `recordState`-less predecessors of the current indexes, superseded when `ADR-005` added that field but never removed — the same 4 already noted, without this detail, in `CURRENT_TASK.md`'s "What is NOT yet decided"; 1 is an unexplained `devices`-collection index that doesn't match the `ADR-007` Device schema at all). More significantly: **no index, deployed or checked-in, covers `status + recordState + userId + receivedAt`** — exactly what the customer-facing (uid-scoped) tab view needs. Predates Phase 2 entirely and isn't worsened by it, but is a real, possibly-live gap. Not fixed blind — recorded as `BACKLOG.md` item 17 with a clear verify-then-decide recommendation. No index or rules changes made.
2. **Permanent `whereIn` compatibility-query test added.** `test/firestore-rules/maintenance-devices-status-compatibility.test.js` (5 tests, reusing the existing emulator harness) seeds both vocabularies and proves the actual Firestore query — not just `DeviceStatus.isInGroup()`'s boolean logic — returns the correct merged set per compatibility group, that unrelated statuses are excluded, and that groups don't leak into each other. Run as a real authenticated staff user, so it also exercises the live security rules.

The Awaiting-Approval-reaches-legacy-Fixed action-gating requirement was recorded directly on `ADR-007`'s Phase 4 rollout bullet (the authoritative source for phase requirements), not just in this log, so it can't be forgotten when capability implementation begins.

**Outcome — merge:** `gh pr merge --squash --delete-branch` again failed on its local SSH-dependent housekeeping step, this time also leaving local `main` briefly checked out to the pre-merge commit with the feature branch already deleted (a `git checkout main` side effect of the same failed cleanup, not data loss — both the merge commit and the pushed branch were already safe on GitHub). Verified via `gh pr view 27 --json state,mergedAt,mergeCommit` before proceeding, per this project's established practice, rather than concluding failure from the error text. Local `main` resynced via HTTPS fetch + `git merge --ff-only` (clean fast-forward, `86ee92f..dc879ee`).

**Testing:** `flutter analyze` — 44 pre-existing issues, zero new (unchanged baseline). `test/core/utils/device_status_test.dart` — 6/6 pass (new, pure-logic coverage for the grouping helper). `test/firestore-rules/` full suite — 91/91 pass (86 existing + 5 new compatibility-query tests). Android emulator smoke test — see above.

**Merged:** PR #27, squash-merged as `dc879ee3685debf3c97316febce38eb66c5ab0a0` to `main`. `feature/adr-007-phase2-bridge-client` deleted both locally and remotely (remote deletion confirmed via an empty `git ls-remote --heads`).

**Deployed:** Nothing. Deliberately not deployed and not distributed — `firestore.rules`, `firestore.indexes.json`, and `functions/` are all untouched (Phase 2 is client-only), and per the clarified rollout, this build is not released to staff independently. It lands in `main` purely as the compatibility foundation for the phases ahead, to ship together in one coordinated transition once that broader body of work is ready.

**Explicitly not decided/not implemented in this session:** Phase 3 (confirm bridge adoption) and Phases 4–8 — not started. `BACKLOG.md` item 17 (the index drift/gap finding) is recorded, not resolved — verification (whether the customer-uid-scoped tab view is a real, currently-working path) and any resulting index change are explicitly deferred to their own follow-up. Stopping here per explicit instruction, before beginning Phase 3.

---

### 2026-07-26 — `ADR-007` Rollout sequence reconciled against the coordinated-release strategy; both capability-client design dependencies resolved

**Decision:** With PR #27 closed, the product owner asked for the Phase-2 rollout clarification (staff stay on the current production build; the bridge client isn't distributed independently) to be reconciled all the way through `ADR-007`'s remaining rollout steps before capability-client work starts — plus closure of the two design dependencies already recorded as blocking a complete capability-client plan. Explicitly scoped as documentation/design-decision work only; no code, no `ADR-007` implementation, this session.

**Outcome — rollout sequence reconciled, in place:** the original 8-step design used two separate confirmed-cutover gates (confirm bridge adoption, then confirm capability adoption) because it assumed the bridge and capability clients would ship as two independent releases. That assumption no longer holds. `ADR-007`'s Rollout section rewritten to 7 steps: additive backend (complete) → bridge compatibility client (complete, merged, intentionally not distributed) → capability-client implementation (developed and merged while production stays on the current build, since nothing is deployed until the next step) → coordinated release/adoption gate (the single real distribution event, standing in for what were separately steps 3 and 5) → tighten rules (only after that gate confirms adoption) → status-vocabulary migration (only after rules tighten) → optional dual-vocabulary cleanup. The old "confirm bridge adoption" step is removed outright, not merely renumbered — it has no execution target once there's no independently-deployed bridge-only fleet to confirm. The closing Scoping note corrected to describe one confirmed-cutover gate, not two.

**Outcome — Phase 2's necessity restated, not merely preserved:** the reconciliation surfaces a sharper version of the original reasoning. At the moment the coordinated build first ships, the status-vocabulary migration hasn't run yet (it's ordered after the adoption gate), so every existing Visit still holds legacy literals. The newly-distributed client's first real job, before touching anything new, is displaying that entire legacy dataset correctly — exactly what Phase 2's `lib/core/utils/device_status.dart` grouping logic already does. Migrating before distribution was walked through as a concrete failure mode, not asserted: it would rewrite literals the *currently-shipped* (pre-Phase-2) production client has never been coded to read, making Visits vanish from its tabs — the exact scenario Phase 2 exists to prevent, defeated by sequencing it wrong.

**Outcome — IMEI/serial normalization and validation policy, resolved:** normalization (for matching) kept strictly separate from validation (for data entry) — a distinction the product owner explicitly asked to be pressure-tested. Normalization strips whitespace/common separators and uppercases (a no-op for all-digit IMEIs, necessary for alphanumeric serials); deliberately no checksum or fixed-length enforcement, since this shop's own problem/accessory lists confirm it services devices beyond phones, and imposing IMEI-specific rigor would incorrectly reject a legitimate non-phone serial. A value that normalizes to empty is treated as absent evidence, never a matchable value — two devices both lacking a recorded IMEI must never appear to match each other. Validation stays light-touch and non-blocking, deliberately contrasted against `phoneNumber`'s existing strict `_normalizePhoneNumber` — phone number is a true Auth-verified identity signal; IMEI/serial is explicitly evidence, not identity, per `ADR-007`'s own framing, and the design does not quietly promote it past that bar. Schema consequence: one new derived field, `devices/{deviceId}.imeiNumberNormalized`, recomputed on every write by a shared pure function, enabling a single-field equality match query that needs no new composite index. Rules consequence: the existing `devices` key-set gains the new field as optional-string-when-present, matching the existing `imeiNumber` check exactly — rules deliberately don't re-derive or verify the normalization itself, consistent with this ADR's existing app-trusted precedent for the status-transition graph and the `finalAmountCharged` ceiling check.

**Outcome — customer-known-Devices query design, resolved:** two reads, not a denormalized shortcut. Read 1: the customer's Visits by `userId`, deliberately including archived ones (device history is historical fact independent of active-workflow visibility) and deliberately omitting a `recordState` filter, which also keeps it a single-field equality query needing no composite index — sidestepping `BACKLOG.md` #17's gap entirely rather than needing it fixed first. A defensive `.limit(200)` guards the pathological case, mirroring `streamDevicesForTab`'s existing `limit(50)` pattern. Client-side dedup into a `Set<String>` (Firestore has no server-side `DISTINCT`). Read 2: batched `documentId() in [...]` fetch in chunks of ≤30, reusing the exact `whereIn` pattern Phase 2 already established, explicitly avoiding the N+1 shape the product owner ruled out. Denormalizing a `knownDeviceIds` array onto `users/{uid}` was considered and rejected on the record — it would reintroduce a second write path that must stay synchronized with the Visits collection, the same drift risk `price`'s own post-mortem (§5) already illustrates, for a lookup that isn't a hot path. No new authorization surface — both reads use Phase 1's already-ratified, already-deployed read rules.

**Outcome — `BACKLOG.md` #17 sequencing confirmed:** does not block capability-client implementation (that work never touches the affected query shape, and the local emulator doesn't enforce the missing index either way). Recorded explicitly as a pre-release readiness item for the new Rollout step 4 (the coordinated release/adoption gate) instead, alongside the manual staff-device check already happening there. Explicitly distinguished from the new customer-known-Devices query, which was deliberately designed to need no new composite index and shares no overlap with #17's gap.

**Documentation reconciled:** `ADR-007` — Rollout section rewritten in place; top Status line updated; §1's `devices` schema block gained `imeiNumberNormalized`; two new dated subsections appended to the existing Device Matching addendum, resolving what its own "recorded as implementation dependencies, not designed here" bullets had flagged. `BACKLOG.md` #17 gained a Sequencing note. `CURRENT_TASK.md` updated in the Active task and "What is NOT yet decided" sections.

**Testing:** Documentation-only change; no schema, Firestore rules, or client/UI code touched; `flutter analyze` not affected.

**Merged:** Committed directly to `main` (documentation only, no PR, matching prior `ADR-007` precedent).

**Explicitly not decided/not implemented in this session:** capability-client implementation itself (Rollout step 3) — not started, per explicit instruction to bring back a complete repository-grounded implementation plan first. `BACKLOG.md` #17's actual verification/fix, and the coordinated release/adoption gate itself, both remain future work tied to Rollout step 4, not this session.

---

### 2026-07-26 — `ADR-007` step 3, PR 1 of 4 (schema/model/service foundation) implemented, reviewed, merged (PR #28)

**Decision:** With the Rollout reconciled and both capability-client design dependencies resolved, the product owner authorized starting step 3 (capability-client implementation), broken into 4 sequential PRs. PR 1 is foundation only — models, path builders, a narrowly-scoped `DeviceServices`, IMEI normalization, and rules — no UI.

**Outcome — a targeted rules refinement before implementation, not assumed:** reviewing the existing `maintenanceDevices` update rule (unchanged since before `ADR-007`, a blanket staff-wide clause with no field-level restriction) against the Final authorization matrix surfaced two rows with no "(from Phase 6)" deferral marker, unlike the status-literal/`finalAmountCharged` rows: `deviceId` immutability ("matches current capability only") and `technicalFinding`/`technicalWorkConcludedAt`'s Maintenance+Admin restriction ("technical judgment can't be borrowed situationally"). Both framed as correctness/authority concerns, not rollout-safety ones — ratified as immediate, not deferred. The `deviceId` expression was refined once more on the product owner's own pressure-test: an initial `.get(field, null)` equality check would have conflated "key absent" with "key present, value null," so it was redesigned on key presence *and* value (`'deviceId' in resource.data` / `'deviceId' in request.resource.data`), keeping legacy Visits and new-world Visits correctly distinct under every combination of add/remove/reassign.

**Outcome — implementation:** `Device`/`Estimate` Dart models plus `FirestoreApiPath` builders (zero client code existed for either collection before this — Phase 1 was rules-only). `MaintenanceDeviceModel` gains `deviceId`/`finalAmountCharged`/`technicalFinding`/`technicalWorkConcludedAt`. `lib/core/utils/imei_normalization.dart` keeps normalization (for matching) strictly separate from validation (for data entry) — no checksum/fixed-length enforcement, since this shop services devices beyond phones and IMEI is evidence, not identity, unlike the strictly-validated `phoneNumber`. `DeviceServices` stays to `createDevice`/`getDeviceById`/`findByNormalizedImei` — `buildImeiFields()` is the sole place `imeiNumber`/`imeiNumberNormalized` are computed together, so no caller can author the normalized field independently, and clearing an IMEI explicitly `FieldValue.delete()`s both rather than leaving a stale normalized value behind. Deliberately no `updateDevice()` method, no customer-known-Devices query, no Estimate service methods — all left for the PRs that actually need them, per the product owner's explicit instruction not to build speculative API surface.

**Outcome — two real issues caught during implementation and independent review, both closed as fixes, not deferred:**
1. While implementing the `deviceId` rules fix, tracing the actual write path (`NewDeviceServices.updateDevice`'s `merge: true`) found that `MaintenanceDeviceModel.toJson()`'s existing unconditional-null pattern (used for every other optional field on the model) would have sent an explicit `deviceId: null` on every edit of every legacy Visit, tripping the new rule and breaking ordinary editing app-wide the instant this merged. Fixed by making `deviceId` the one field `toJson()` omits entirely rather than writing null.
2. An independent review pass, fresh against the actual diff rather than repeating earlier design notes, generalized this: `finalAmountCharged`/`technicalFinding`/`technicalWorkConcludedAt` had the same unconditional-null pattern. Since no write path sets any of the three yet, every caller that reconstructs a model for an unrelated edit — including `new_device_maintenance.dart`'s `onSaveLogic()`, untouched by this PR and unaware these fields exist — defaults them to null; left unconditional, a future PR's first write to any of them would be silently erased by the next unrelated Maintenance/Admin edit through the existing screen (Reception would at least get a hard rejection from the authority check; Maintenance/Admin would not, since they're authorized). The product owner explicitly chose to close this now at the serialization layer, the same way as `deviceId`, rather than leave it as a comment for PR 3/4 to remember. The same review pass also found the two new rules constraints, though correctly placed globally across all three update-rule branches, had no executable proof a *bundled* write (e.g. Restore + reassign `deviceId`, or Archive + set `technicalFinding`) was actually caught — closed with two new tests rather than left to structural confidence.

**Testing:** `flutter analyze` — 44 pre-existing issues, zero new, across both rounds. Dart unit tests — 26/26 final (7 IMEI normalization, 5 `DeviceServices.buildImeiFields`, 6 pre-existing `DeviceStatus`, 3 `deviceId` serialization regression, 5 `finalAmountCharged`/`technicalFinding`/`technicalWorkConcludedAt` serialization regression). `test/firestore-rules/` full suite — 112/112 final (91 pre-PR + 21 new: 3 `imeiNumberNormalized` shape cases, 15 in a new `maintenance-devices-adr007-fields.test.js` covering both integrity constraints across every role/state combination including the two bundled-transition cases, 3 extending `maintenance-devices-regression.test.js` with previously-uncovered ordinary-edit and frozen-archived cases). The historical 74-case authorization matrix (`BACKLOG` #0a) was checked directly rather than assumed reproducible — confirmed it was never committed to the repository and no longer exists as a runnable artifact, so per the product owner's own stated fallback, deployment stays deferred by default rather than treating partial-equivalent coverage as sufficient to authorize it early.

**Outcome — merge:** `gh pr merge --squash --delete-branch` failed on its usual local SSH-dependent housekeeping step, the same pattern as every prior merge on this repo. Verified via `gh pr view 28 --json state,mergedAt,mergeCommit` before proceeding: `state: MERGED`, commit `3dc06665db68455600d1772fc1aea2677efeb708`. Remote branch deletion confirmed via an empty `git ls-remote --heads`. Local `main` resynced via HTTPS fetch + `git merge --ff-only` (clean fast-forward, `30387d2..3dc0666`).

**Merged:** PR #28, squash-merged as `3dc06665db68455600d1772fc1aea2677efeb708` to `main`. `feat/adr007-phase4-pr1-foundation` deleted both locally and remotely.

**Deployed:** Nothing. The `firestore.rules` change merges to `main` but stays undeployed — same posture as Phase 2 — deferred to `ADR-007`'s coordinated release/adoption gate (Rollout step 4), per explicit instruction.

**Explicitly not decided/not implemented in this session:** step 3's PRs 2–4 (Device matching/selection UI, Estimate creation/decision/resolution plus the legacy compatibility-tab action-gating fix, Technical Finding/technical-work-concluded-at flow) — not started, per explicit instruction to stop after bookkeeping. Steps 4–7 (coordinated release/adoption gate, rules tightening, data migration, optional cleanup) also not started.
